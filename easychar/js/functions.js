...
jQuery($=> {
    ...
    $("#submit_btn1 , #submit_btn").on('click', function () {
        ...
        grecaptcha.ready(function() {
            ...
            grecaptcha.execute('6LdIH-IUAAAAABdUSl1EVBQ_HCSotDaXdtT-Bf9e', {action: 'contact'}).then(function(token) {
                // add token to form
               // console.log(token);
                $('#getin_form').prepend('<input type="hidden" name="g-recaptcha-response" value="' + token + '">');
    ...
    $("#services-measonry2").cubeportfolio({
        layoutMode: 'grid',
        defaultFilter: '*',
        filters: '#services-filter',
        animationType: "scaleSides",
...
{
    $(".enbtext").slideUp(1000);
    $("#clk"+id).slideDown(1000);
}