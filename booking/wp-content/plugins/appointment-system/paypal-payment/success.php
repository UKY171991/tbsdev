...
if(!empty($_GET['item_number']) && !empty($_GET['tx']) && !empty($_GET['amt']) && !empty($_GET['cc']) && !empty($_GET['st'])){ 
    ...
    $payment_status = $_GET['st']; 
     ...
     if($wpdb->num_rows > 0){ 
		...
		$query="INSERT INTO  $ap_payment (item_number,booking_id,txn_id,payment_amount,payment_currency,payment_status) VALUES ('$item_number','$item_name','$txn_id','$payment_gross','INR','$payment_status')";
		$wpdb->query($query);
...
$results = $wpdb->get_results("SELECT * from  $ap_appointment where id = '".$booking_id."'"); 
...
<div class="container">
    ...
    <a href="<?php home_url()?>/" class="btn-link">Back to Home</a>
</div>
